...
final class PrefetchManager: ObservableObject {
    ...
    private func check(model: Model) async {
        ...
        // Robust offline check by scanning BOTH the new AppSupport cache and the legacy ~/.cache
        let script = """
        import os, sys
        ...
        # 2. Legacy Default Cache (~/.cache/huggingface/hub)
        home = os.path.expanduser('~')
        default_cache = os.path.join(home, '.cache', 'huggingface', 'hub')
        paths_to_check.append(default_cache)
