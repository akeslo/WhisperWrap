...
class ContentViewModel: ObservableObject {
    ...
    private func runWhisper(on url: URL, model: Model, format: String, testMode: Bool) async throws -> URL {
        ...
        if testMode {
            ...
            command = "\"\(pythonEnv.pythonPath)\" -c \"from faster_whisper import WhisperModel; import sys; model = WhisperModel('" + modelSize + "', device='auto', compute_type='int8'); segments, info = model.transcribe('" + url.path.replacingOccurrences(of: "\"", with: "\\\"") + "', beam_size=1); next(segments, None); print('OK')\""
        } else {
